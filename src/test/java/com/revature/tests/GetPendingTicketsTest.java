...
class GetPendingTicketsTest {
	...
	@Test
	public void getPendingTicketsTest() {
		...
		List<Tickets> pendingTickets = this.ticketService.getPendingTickets();
		Assertions.assertEquals(pendingTickets.size()==2, true);
		
	}
	
	@Test
	public void getPendingTicketsTest2() {
		List<Tickets> dummyTickets = Arrays.asList(
		...
		Mockito.when(this.ticketRepository.findAll()).thenReturn(dummyTickets);
		List<Tickets> pendingTickets = this.ticketService.getPendingTickets();
		Assertions.assertEquals(pendingTickets.size()==1, true);
